...
BaseType_t aws_mqtt_connect(MQTTContext_t *pxMQTTContext, NetworkContext_t * pxNetworkContext)
{
    ...
    /* Connect to a MQTT host  */
     PlaintextTransportStatus = Plaintext_FreeRTOS_Connect(pxNetworkContext, EXAMPLE_MQTT_HOST , EXAMPLE_MQTT_HOST_PORT,
                                                           MQTT_RX_TIMEOUT_MS,
                                                           MQTT_TX_TIMEOUT_MS);

     FSP_ERROR_RETURN((PlaintextTransportStatus == PLAINTEXT_TRANSPORT_SUCCESS), pdFAIL);
