...
void TMF8828_enable(void);
void TMF8828_startMeasuring();
void TMF8828_enable();
int TMF8828_update(int *conf, int *dist, int *subcapture_nr);
...
void TMF8828_enable() {
    ...
    wire_read_register(ENABLE_REG, i2c_buff_rx, 1);
    if (available()) {
        aux = read();
    }
    if (aux == 97) {
...
bool TMF8828_factoryCalibration(int8_t conf, bool show) {
    ...
    beginTransmission(0x41);
    wire_read_register(0x07, i2c_buff_rx, 1);
    int aux = 0x31;
    if (available()) {
        aux = read();
    }
...
bool TMF8828_update8x8(int obj0conf[][8], int obj0dist[][8], int obj1conf[][8], int obj1dist[][8]) {
    ...
    while (idx < 4) {
    	if (TMF8828_update(conf, dist, &subcapture_nr)) {
    		idx++;
    		delay(25);
