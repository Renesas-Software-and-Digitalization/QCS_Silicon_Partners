...
#define I2C_BUFF_TX_SIZE		64  /*I2C Transmission Buffer Size*/
...
static fsp_err_t wire_init_communication(uint8_t address) {
    ...
    if(err == FSP_SUCCESS) {
        err = g_i2c0.p_api->slaveAddressSet(g_i2c0.p_ctrl, address, I2C_MASTER_ADDR_MODE_7BIT);
        if (FSP_SUCCESS != err) {
...
static fsp_err_t wire_read(uint8_t* buff, uint32_t len, bool restart) {
	...
	if (err == FSP_SUCCESS) {
	    ...
	    APP_ERR_PRINT("** wire_clear_buffer_n failed ** \r\n");
	}
    return err;
}
...
static fsp_err_t wire_read_register(uint8_t reg_addr, uint8_t* buff, uint32_t len) {
    ...
    if(err == FSP_SUCCESS) {
        ...
        APP_ERR_PRINT("** wire_read_register failed ** \r\n");
    }
    return err;
}
